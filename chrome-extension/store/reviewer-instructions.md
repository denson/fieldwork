...
The workspace controls are folded into a short text disclosure above the message box. Open **Workspace** (or **Workspace connected**) to reveal the available text actions. It should remain folded during ordinary chat and close after successfully preparing a mode-choice message. Drafts should look like ordinary readable text in both light and dark chat themes, with an underlined **Use this draft** action.

First verify **Open workspace** from a new chat: after loading the extension, open the Business Plan profile and choose Start New Chat. Without refreshing the new chat, expand Workspace and click Open workspace. With Chrome's blank split-view chooser beside the chat, the workspace should open there and the status should become Workspace connected. Without a blank paired pane, it should open a new tab while preserving other pages. The new-chat routing fix introduced in 0.10.2 is retained.
...
4. Expand **Workspace connected** and choose **Use the connected workspace**. A visible connection report and mode choice should be prepared in the message box, and the controls should fold away. No message should be sent. Review it, then press Send.
...
6. Ask the guide: “Keep the yard-care idea. Rename it Cedar Yard Care and prepare the idea step for the connected workspace.” The guide should produce readable draft fields as ordinary text. Choose **Use this draft** and confirm the displayed name and idea appear in that website step. The website remains editable.
...
All 59 repository tests pass, including the new-chat routing regression against source and packaged background code. Automated verification also covers the store manifest/icons/runtime, public routes, transfer boundaries, races, existing drafts, note validation and unchanged business planning behavior. A live 0.10.4 test passed on September 15, 2026: note placement in the exact paired chat, manual Send, plain-text revision action, complete comparison, cancellation, explicit load, clearing obsolete numbers and restoration of all 25 original fields. The prepared store screenshot shows the unchanged website-side note control, captured with the earlier 0.9.0 extension; it does not assert that the 0.10.4 candidate has been loaded.
