# Fieldwork First Steps — owner-approved companion reference (v4)
...
2. Website to chat: with Fieldwork Companion Pane enabled, Put note in BoodleBox transfers exactly the displayed note to the paired matching guide's empty message box. It never submits or overwrites a draft. The learner reviews the text and presses Send on the left to approve sharing (human in the loop, HITL). Without that button, use the small copy icon above the note, paste into the chat, and Send. Keep the same conversation throughout this practice.
...
4. The website shows the explorer pass. The learner reports whether the guide got both choices right and answers how the guide learned them: the note they sent in the chat. Then the website offers Open the portfolio. This opens the activity menu and Portfolio Guide together when the extension is enabled. The portfolio can remember the selected topic as an interest; it is not a score, proof of completion, or shared answers. Choose an activity there to open its matching guide and website. The site cannot verify chat participation or learning. Website-only exploration is possible.
...
Public website: the plain homepage https://denson.github.io/fieldwork/ opens https://denson.github.io/fieldwork/start.html . New visitors start here, then go to the portfolio at https://denson.github.io/fieldwork/?demo=home . Returning visitors may skip the practice. The website and teaching references are publicly hosted on GitHub Pages. Learner notes remain in the browser until deliberately shared.
...
If Put note in BoodleBox is absent, use the copy icon above the note, click the left message box, paste (Ctrl+V on Windows, Command+V on Mac), and Send. If a long paste becomes attached text, type Here is my practice note and send. If the extension reports a draft already exists, the learner must send or clear that draft before trying again. If it reports the wrong guide, open the matching companion beside the activity.
...
Give [Explore the Fieldwork portfolio](https://denson.github.io/fieldwork/?demo=home) when the learner is ready to continue. Do not interrupt an unfinished practice-note exchange. The portfolio shows all four activities; do not require another quiz or send the learner straight to the selected subject unless explicitly requested.
