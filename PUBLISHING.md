...
Canonical Business Plan bot instructions are v1.5. Business Plan First Steps supports a complete chat experience and optional website support; the other bots retain their existing website activities. For this version, populate the BoodleBox Instructions field with `business-plan-bot-instructions.md`, followed by a clearly labeled current-reference appendix containing `business-plan.md`. The combined text must stay below the builder's 50,000-character limit. This makes the current teaching material available without website retrieval or a separate file upload. The instructions explicitly disregard the older attached snapshot, which remains in BoodleBox rather than being deleted. Also update the greeting and profile description, publish and verify no unpublished changes. Use a new conversation to test chat-only drafting and a brief, numerical explanation, and the optional website transfer. Other Fieldwork bots have not been republished as part of this release.
