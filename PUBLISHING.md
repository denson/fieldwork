...
The extension stages whole-plan proposals through a small Review revised plan action in chat; the rich comparison and save/load controls stay on the website. Single-step transfers are unchanged. Permissions are unchanged. Manual copied-text and backup-file review also work. Bot v1.11 was published and verified after reload, retaining the existing greeting and all five complete examples. Its combined LF instructions contain 48,979 characters (49,353 in the builder).

All 59 repository tests and actual website/content-script DOM fixtures pass. Chrome verified the website’s save, complete comparison, explicit load, restoration and direct revision link. A live bot proposal passed the strict 25-field validator and opened the complete website comparison; its missing navigation guidance prompted an explicit instruction before every revision block. The repeated live test included the save–review–load instruction and correct revision link. The 0.10.4 store ZIP was built and validated. After the Windows restart, live Chrome reported 0.10.4 and the paired-page flow passed: saved original, full request placement with current connection report, manual Send, plain-text Review revised plan action, complete 25-field comparison, cancellation, explicit load, all seven outdated numeric assumptions cleared, and restoration matching all 25 original fields. The original remains loaded, with the revised proposal staged for user review. Nothing has been submitted to Google.
